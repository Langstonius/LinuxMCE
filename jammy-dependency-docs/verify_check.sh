...
ORIG_COUNT=$(cat ../src/Ubuntu_Helpers_NoHardcode/conf-files/jammy-amd64/build-packages | wc -l)
echo "Original file line count: $ORIG_COUNT"

# Count lines in updated packages
UPDATED_COUNT=$(cat ../updated_build_packages | grep -v "^$" | grep -v "^# Updated" | grep -v "^# Original" | grep -v "^# Modified" | wc -l)
echo "Updated file active line count: $UPDATED_COUNT"

# Count commented out packages (replacements)
COMMENTED_COUNT=$(cat ../updated_build_packages | grep "^# .*-" | wc -l)
echo "Commented packages (with replacements): $COMMENTED_COUNT"

# Count packages in missing list
MISSING_COUNT=$(cat ../missing_packages.txt | grep -v "^Packages" | grep -v "^$" | wc -l)
...
for pkg in "libkonq5-dev" "libqt4-dev" "dh-systemd" "python3-oauth"; do
  if grep -q "$pkg" ../missing_packages.txt; then
    echo "✓ $pkg properly identified as missing"
  else
    echo "✗ $pkg not found in missing_packages.txt"
  fi
  
  if grep -q "$pkg" ../replacement_suggestions.txt; then
