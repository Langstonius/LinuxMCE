...
//================================================================page output=========================	
	...
	$fileResult = $mediadbADO->Execute($fileQuery);
	if (!$fileResult) {
		die("Database error: " . $mediadbADO->ErrorMsg());
	}
	$fileData = $fileResult->FetchRow();		
	$queryAttributes='
		...
		$rowData = $mediadbADO->Execute($queryAttributes);
		if (!$rowData) {
			die("Database error: " . $mediadbADO->ErrorMsg());
		}
		$pos=0;
		while($rowAttributes=$rowData->FetchRow())
		{	$pos++;
			$out.='
